...
switch ($action) {
			...
			if($sql->execute(array("cancelled",$la_empno,$la_id))){
				foreach ($con1->query("SELECT * FROM tbl201_leave JOIN tbl_timeoff ON timeoff_name=la_type WHERE la_id=$la_id") as $v) {
					...
					if(($la_type=="Leave Without Pay" || $la_type=="Sick Leave")  && isset($v['la_dates'])){
						...
						$sql2->execute(array($la_empno, $curdt, date("Y-m-d",strtotime($startdt." +".($days-1)." days")), $la_type));
					}
				}
				echo "1";
	...
	// ot
		case 'approve ot':
			...
			// if($sql->execute(array("approved",$user_empno,$sign,date("Y-m-d"),$ot_empno,$id))){
			// 	echo "1";
			...
			try {
				...
				if($sql->execute(array("confirmed",$user_empno,$sign,date("Y-m-d"),$user_empno,$dt_confirm,$ot_empno,$id))){
				...
				*/
				...
				if(!empty($data)){
					...
					foreach ($data as $k => $v) {
						...
						if($sql->execute([ date("Y-m-d H:i:s"), $user_empno, $sign, $empno, $id ])){
							...
							$sql->execute([ date("Y-m-d"), $user_empno, date("Y-m-d"), $user_empno, $sign, $empno, $id ]);
						}
					}
					echo "1";
				}else{
		...
		case 'confirm ot':
			...
			try {
				...
				if($sql->execute(array("confirmed",$user_empno,$dt_confirm,$ot_empno,$id))){
		...
		case 'deny ot':
			if(!empty($_POST['data'])){
				foreach ($_POST['data'] as $k => $v) {
					...
					if($sql->execute(array("Denied",$user_empno,$ot_empno,$id))){
						...
						$sql->execute([ $user_empno, $ot_empno, $id ]);
					}
				}
				echo "Denied";
			}else{
	...
	// offset
		case 'approve offset':
			...
			if(!empty($data)){
				foreach ($data as $k => $v) {
					...
					if($sql->execute(array("approved",$user_empno,$sign,date("Y-m-d"),$os_empno,$id))){
		...
		case 'confirm offset':
			...
			try {
				...
				if($sql->execute(array("confirmed",$user_empno,$dt_confirm,$os_empno,$id))){
		...
		case 'deny offset':
			if(!empty($_POST['data'])){
				foreach ($_POST['data'] as $k => $v) {
					...
					if($sql->execute(array("denied",$user_empno,$os_empno,$id))){
	...
	// activity
		...
		case 'approve training':
			...
			if(!empty($data)){
				...
				foreach ($data as $rk => $rv) {
					...
					if($sql->execute(array("CONFIRMED",$user_empno,$sign,date("Y-m-d H:i:s"),$user_empno,date("Y-m-d H:i:s"),$empno,$id))){
		...
		case 'confirm training':
			...
			$sql=$con1->prepare("UPDATE tbl_edtr_hours SET dtr_stat=?, confirmedby=?, confirmed_dt=?, date_added = date_added WHERE emp_no=? AND id=?");
				if($sql->execute(array("CONFIRMED",$user_empno,$dt_confirm,$empno,$id))){
		...
		case 'deny training':
			...
			if(!empty($_POST['data'])){
				...
				foreach ($data as $rk => $rv) {
					...
					if($sql->execute(array("DENIED",$user_empno,$dt_deny,$reason,$empno,$id))){
	...
	// drd
			...
			if(!empty($data)){
				...
				try {
					...
					foreach ($data as $k => $v) {
						...
						if($sql->execute(array("confirmed",$user_empno,$sign,date("Y-m-d"),$user_empno,$dt_confirm,$drd_empno,$id))){
				...
				// if($sql->execute(array("approved",$user_empno,$sign,date("Y-m-d"),$drd_empno,$id))){
				// 	echo "1";
		...
		case 'confirmed drd':
			...
			try {
				...
				if($sql->execute(array("confirmed",$user_empno,$dt_confirm,$drd_empno,$id))){
		...
		case 'deny drd':
			...
			if(!empty($_POST['data'])){
				foreach ($_POST['data'] as $k => $v) {
					...
					if($sql->execute(array("denied",$user_empno,$drd_empno,$id))){
	...
	// dhd
			...
			if(!empty($data)){
				...
				try {
					...
					foreach ($data as $k => $v) {
						...
						$sql=$con1->prepare("UPDATE tbl201_dhd SET dhd_status=?, dhd_approvedby=?, dhd_signature=?, dhd_approveddt=?, dhd_confirmedby=?, dhd_confirmeddt=? WHERE dhd_empno=? AND dhd_id=?");
						if($sql->execute(array("confirmed",$user_empno,$sign,date("Y-m-d"),$user_empno,$dt_confirm,$dhd_empno,$id))){
				...
				// if($sql->execute(array("approved",$user_empno,$sign,date("Y-m-d"),$dhd_empno,$id))){
				// 	echo "1";
		...
		case 'confirmed dhd':
			...
			try {
				...
				if($sql->execute(array("confirmed",$user_empno,$dt_confirm,$dhd_empno,$id))){
		...
		case 'deny dhd':
			...
			if(!empty($_POST['data'])){
				foreach ($_POST['data'] as $k => $v) {
					...
					if($sql->execute(array("denied",$user_empno,$dhd_empno,$id))){
		...
		case 'approve restday':
			if(!empty($_POST['data'])){
				foreach ($_POST['data'] as $k => $v) {
					...
					if($sql->execute(array($user_empno, date("Y-m-d"), $empno, $id))){
						// echo "1";
		...
		case 'approve gatepass':
			...
			if(!empty($_POST['data'])){
				...
				foreach ($_POST['data'] as $k => $v) {
					...
					if($sql->execute(array('APPROVED',$user_empno,date("Y-m-d H:i:s"),$latefile,$id1))){
						if($latefile>0){
		...
		case 'deny dtr':
			if(!empty($_POST['data'])){
				foreach ($_POST['data'] as $k => $v) {
					...
					if($sql->execute(array('DENIED',$user_empno,date("Y-m-d H:i:s"),$id1))){
