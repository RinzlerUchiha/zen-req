...
function get_leave_no(){
	global $con1;
	$query = $con1->query("SELECT la_no FROM tbl201_leave order by CAST(la_no AS UNSIGNED) DESC limit 1");
...
switch ($action) {
	case 'add':
			...
			if($la_type=='Maternity Leave'){
				$la_end=date("Y-m-d",strtotime($la_start." +".($la_days-1)." days"));
			}else{
				$la_end=end($enddt);
				$la_mtype = '';
			}
	...
	case 'batch_approve':
		    ...
		    if (!$ids || !$emps || !$signature) {
		        echo "Missing required batch data.";
		        break;
		    }
		
		    $ids_array = explode(',', $ids);
		    $emps_array = explode(',', $emps);
		
		    try {
		        $con1->beginTransaction();
		        $sql = $con1->prepare("UPDATE tbl201_leave 
		            SET la_status = ?, la_approvedby = ?, la_signature = ?, la_approveddt = ? 
		            WHERE la_empno = ? AND la_id = ?");
		            
		        for ($i = 0; $i < count($ids_array); $i++) {
		            $la_id = $ids_array[$i];
		            $la_empno = $emps_array[$i];
		            
		            // Perform the Update
		            $sql->execute(["approved", $user_id, $signature, date("Y-m-d"), $la_empno, $la_id]);
		
		            // Optional: Get details for logging
		            $startdt = '';
		            $enddt = '';
		            $la_type = '';
		            
		            $details = $con1->prepare("SELECT * FROM tbl201_leave 
		                                        JOIN tbl_timeoff ON timeoff_name = la_type 
		                                        WHERE la_id = ?");
		            $details->execute([$la_id]);
		            
		            if ($la_key = $details->fetch(PDO::FETCH_ASSOC)) {
		                $startdt = $la_key['la_start'];
		                $la_type = $la_key['la_type'];
		                $enddt = date("Y-m-d", strtotime($startdt . " +100 days"));
		        ...
		        $con1->commit();
		        echo "Batch approved $count request(s).";
	...
	case 'return':
		...
		$leave_id = $_POST['l_id'];
		$empno = $_POST['empno'];
		$rtw_end = $_POST['end_date'];
		$rtw_return = $_POST['return_date'];

		$sql=$con1->prepare("SELECT COUNT(*) FROM tbl_return_to_work WHERE rtw_leaveid = ? AND rtw_id != ?");
		$sql->execute([ $leave_id, $rtw_id ]);
		if(!empty($sql->fetch(PDO::FETCH_NUM)[0])){
			echo "2";
			exit;
		}
		...
		try {
			...
			if($rtw_id){
				$sql=$con1->prepare("UPDATE tbl_return_to_work SET rtw_end = ?, rtw_returndate = ?, rtw_timestamp = ? WHERE rtw_id = ? AND rtw_leaveid = ? AND rtw_empno = ?");
				$sql->execute([ $rtw_end, $rtw_return, date('Y-m-d H:i:s'), $rtw_id, $leave_id, $empno ]);
				echo "1";
			}else{
				$sql=$con1->prepare("INSERT INTO tbl_return_to_work (rtw_empno, rtw_leaveid, rtw_end, rtw_returndate, rtw_timestamp) VALUES (?, ?, ?, ?, ?)");
				$sql->execute([ $empno, $leave_id, $rtw_end, $rtw_return, date('Y-m-d H:i:s') ]);
				echo "1";
			}
	...
	case 'approve-rtw':
		...
		try {
			...
			$sql=$con1->prepare("UPDATE tbl_return_to_work SET rtw_stat = 'approved', rtw_approvedby = ?, rtw_approveddate = ? WHERE rtw_id = ? AND rtw_leaveid = ? AND rtw_empno = ?");
			if($sql->execute([ $user_id, date('Y-m-d'), $rtw_id, $leave_id, $empno ])){
				$sql2 = $con1->prepare("SELECT * FROM tbl_return_to_work LEFT JOIN tbl201_leave ON la_id = rtw_leaveid WHERE rtw_id = ? AND rtw_leaveid = ? AND rtw_empno = ?");
				$sql2->execute([ $rtw_id, $leave_id, $empno ]);
				foreach ($sql2->fetchall(PDO::FETCH_ASSOC) as $k => $v) {

					$sql3 = $con1->prepare("UPDATE tbl_edtr_hours SET dtr_stat = 'CANCELLED' WHERE emp_no = ? AND date_dtr BETWEEN ? AND ? AND day_type = ? AND date_dtr <= ?");
					$sql3->execute([ $empno, $v['la_start'], $v['la_end'], $v['la_type'], $v['rtw_end'] ]);

					$filtered_dates = [];
					foreach (explode(",", $v['la_dates']) as $dt) {
						if ($dt <= $v['rtw_end']) {
							$filtered_dates[] = $dt;
						}
					}
					
					$la_dates_str = implode(",", $filtered_dates);
					$la_days = count($filtered_dates);

					if($v['la_type'] == 'Maternity Leave'){
						// Create DateTime objects for the two dates
						$date1 = new DateTime($v['rtw_end']);
						$date2 = new DateTime($v['la_end']);

						// Calculate the difference between the two dates
						$interval = $date1->diff($date2);
						$la_days = $la_days - $interval->days;
					}
					
					$sql4 = $con1->prepare("UPDATE tbl201_leave SET la_dates = ?, la_end = ?, la_return = ?, la_days = ? WHERE la_id = ? AND la_empno = ?");
					$sql4->execute([ $la_dates_str, $v['rtw_end'], $v['rtw_returndate'], $la_days, $leave_id, $empno ]);

					$trans->_log("Update leave info ID: $leave_id, la_dates: $la_dates_str, la_end: ".$v['rtw_end'].", la_return: ".$v['rtw_returndate'].", la_days: $la_dates_str");
	...
	case 'deny-rtw':
		...
		try {
			...
			$sql=$con1->prepare("UPDATE tbl_return_to_work SET rtw_stat = 'cancelled', rtw_approvedby = ?, rtw_approveddate = ? WHERE rtw_id = ? AND rtw_leaveid = ? AND rtw_empno = ?");
			if($sql->execute([ $user_id, date('Y-m-d'), $rtw_id, $leave_id, $empno ])){
