...
try {
    ...
    $outlet_pcf = $_POST['outlet_pcf'];
    $status_pcf = $_POST['status_pcf'];
    ...
    // if ($count > 0) {
    //     echo json_encode(["success" => false, "error" => "Empty PCV detected! Entry not saved. Please reload page."]);
    //     exit;
    // }
    ...
    $query = "INSERT INTO tbl_disbursement_entry 
        (dis_no, dis_date, dis_outdept, dis_empno, dis_pcv, dis_or, dis_payee, 
         dis_office_store, dis_transpo, dis_repair_maint, dis_commu, dis_misc, dis_total, dis_replenish_no, dis_status) 
    ...
    $stmt->bindValue(":dis_replenish_no", $outlet_pcf, PDO::PARAM_STR);
    $stmt->bindValue(":dis_status", $status_pcf, PDO::PARAM_STR);
    $stmt->bindValue(":dis_total", $total, PDO::PARAM_STR);
