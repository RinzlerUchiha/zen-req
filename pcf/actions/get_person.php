<?php
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        $stmt = $hr_db->prepare("SELECT 
                ...
                headinfo.`pi_cmobileno` AS headphone,
                empinfo.`pi_mobileno` AS empcontact,
                b.jrec_reportto,
                b.jrec_outlet,
                b.jrec_department,
                b.jrec_position,
            ...
            LEFT JOIN
                tbl201_persinfo headinfo ON headinfo.`pi_empno` = head.`bi_empno`
            LEFT JOIN 
                tbl_jobdescription jd ON jd.jd_code = b.jrec_position
            LEFT JOIN 
                tbl201_jobinfo ji ON ji.ji_empno = a.bi_empno
            LEFT JOIN 
                tbl201_persinfo empinfo ON empinfo.pi_empno = a.bi_empno
            WHERE 
                a.bi_empno = :user_id
        ...
        if ($user) {
            ...
            $date = date('F j, Y');
            $headcontact = $user['headphone'];
            $empcontact = $user['empcontact'];
            // echo $headcontact;
