...
$userinfo['department'] = '';
$userinfo['company'] = '';
$sql = "SELECT * FROM tbl201_jobrec WHERE jrec_empno = ?";
$query = $con1->prepare($sql);
$query->execute([ $user_id ]);
foreach ($query->fetchall(PDO::FETCH_ASSOC) as $k => $v) {
  $userinfo['department'] = $v['jrec_department'];
  $userinfo['company'] = $v['jrec_company'];
}
...
if(in_array($userinfo['company'], ['TNGC', 'STI', 'SJI']) && $userinfo['department'] != 'SLS' && $current_path == '/zen/ot/'){
  exit;
}
  ...
  <div class="page-header" style="margin-bottom:0px !important;">
    ...
    <div class="page-header-breadcrumb">
      <ul class="breadcrumb-title">
        ...
        <li class="breadcrumb-item"><a href="#!">DHD</a></li>
      </ul>
    </div>
  </div>
  <div class="page-body">
                ...
                <div class="header-fun">
                  <div class="sub-buttons">
                    ...
                    <?php echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_id."\" data-reqchange=\"0\" data-target=\"#".$load."modal\"><i class='fa fa-plus'></i></button>";
                      $tochk = 0;
                      // if($tochk > 0){
                        echo "<button type='button' class='btn btn-outline-primary btn-sm float-right mt-2 batchapprove' data-reqtype='".$load."'>Approve selected</button>";
                      // }
                    ?>
                  </div>
                  <div class="sub-date">
                          ...
                          if(!empty($arr['pending'])){
                            foreach ($arr['pending'] as $k => $v) {
                              ...
                              if($v['empno'] != $user_id && in_array($v['empno'], $user_assign_arr2)){
                                ...
                                echo "<button type=\"button\" class=\"reqdeny btn btn-outline-danger btn-sm m-1\" data-toggle=\"modal\" data-target=\"#deny-reason\"  title='Deny' data-reqtype=\"".$load."\" data-reqid=\"".$k."\" data-reqemp=\"".$v['empno']."\"><i class='fa fa-times'></i></button>";
                              }
                              echo "</td>";
                              echo "</tr>";
                            }
...
<div class="modal fade" id="dhdmodal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="dhdModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="form_dhd" style="padding: 20px;">
...
<div class="modal fade" id="deny-reason" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body p-3">
                <input type="text" class="form-control" id="dhd-reason">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default waves-effect btn-mini" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary waves-effect btn-mini" id="save-deny">Save</button>
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function () {
  $(document).on("change", ".approvechkall", function () {
    ...
    $(".approvechkitem").prop("checked", isChecked);
  });

  $(document).on("change", ".approvechkitem", function () {
    if (!$(this).prop("checked")) {
      $(".approvechkall").prop("checked", false);
    } else {
      if ($(".approvechkitem:checked").length === $(".approvechkitem").length) {
        $(".approvechkall").prop("checked", true);
      }
    }
  });
});
  ...
  $(function(){
    ...
    $(document).on('click', '.batchapprove', function () {
      ...
      $.ajax({
        ...
        data: {
          action: 'approve dhd',
          batchdata: JSON.stringify(selected)
        },
        success: function (res) {
          if (res == "1") {
            alert("Approved successfully!");
    ...
    $(document).on('click', '.reqdeny', function () {
      denyData.empno = $(this).data('reqemp');
      denyData.id = $(this).data('reqid');
      denyData.type = $(this).data('reqtype');
    
      $('#dhd-reason').val('');
    });
    
    $(document).on('click', '#save-deny', function () {
      const reason = $('#dhd-reason').val().trim();
      if (reason === "") {
