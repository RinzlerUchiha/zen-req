...
switch ($load) {
	case 'month':
		...
		if($e == 'all' && $trans->get_assign('manpower', 'viewemp', $user_id)){
			$e = $user_assign_list;
		}
			...
			for ($x = 0; $x < 7; $x++) { 
				...
				if($days[$x] == date("D", strtotime($curdt)) && $curmonth == date("F", strtotime($curdt)) && $curdt <= $enddt){
					...
					if($e == "all" || $e == "" || count(explode(",", $e)) > 1){
						...
						$otcnt = count(array_filter($dtr_ot, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr2, $user_id) {
						...
						$leavecnt = count(array_filter($leavearr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr2, $user_id) {
									    return isset($v[$curdt]) && $v[$curdt]['status'] == 'pending' && in_array($k, $user_assign_arr2) && ((isset($prev_outlet[$k]) && $o == $prev_outlet[$k]) || (isset($targets[$k][date("Y-m", strtotime($curdt))]) && $o == $targets[$k][date("Y-m", strtotime($curdt))]) || $o == '');
									}, ARRAY_FILTER_USE_BOTH));
						$travelcnt = count(array_filter($travelarr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr3, $user_id) {
						...
						$trainingcnt = count(array_filter($trainingarr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr3, $user_id) {
									    return isset($v[$curdt]) && $v[$curdt]['status'] == 'pending' && in_array($k, $user_assign_arr3) && ((isset($prev_outlet[$k]) && $o == $prev_outlet[$k]) || (isset($targets[$k][date("Y-m", strtotime($curdt))]) && $o == $targets[$k][date("Y-m", strtotime($curdt))]) || $o == '');
									}, ARRAY_FILTER_USE_BOTH));
						$oscnt = count(array_filter($osarr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr2, $user_id) {
									    return isset($v[$curdt]) && $v[$curdt]['status'] == 'pending' && in_array($k, $user_assign_arr2) && ((isset($prev_outlet[$k]) && $o == $prev_outlet[$k]) || (isset($targets[$k][date("Y-m", strtotime($curdt))]) && $o == $targets[$k][date("Y-m", strtotime($curdt))]) || $o == '');
									}, ARRAY_FILTER_USE_BOTH));
						$drdcnt = count(array_filter($drdarr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr2, $user_id) {
									    return isset($v[$curdt]) && $v[$curdt]['status'] == 'pending' && in_array($k, $user_assign_arr2) && ((isset($prev_outlet[$k]) && $o == $prev_outlet[$k]) || (isset($targets[$k][date("Y-m", strtotime($curdt))]) && $o == $targets[$k][date("Y-m", strtotime($curdt))]) || $o == '');
									}, ARRAY_FILTER_USE_BOTH));
						$dhdcnt = count(array_filter($dhdarr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr2, $user_id) {
									    return isset($v[$curdt]) && $v[$curdt]['status'] == 'pending' && in_array($k, $user_assign_arr2) && ((isset($prev_outlet[$k]) && $o == $prev_outlet[$k]) || (isset($targets[$k][date("Y-m", strtotime($curdt))]) && $o == $targets[$k][date("Y-m", strtotime($curdt))]) || $o == '');
									}, ARRAY_FILTER_USE_BOTH));
	...
	case 'dtrreport':
		...
		if($emp_arr == 'all' && $trans->get_assign('manpower', 'viewemp', $user_id)){
	...
	case 'leave':
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				...
				if($v['la_empno'] != $user_id && in_array($v['la_empno'], $user_assign_arr2)){
		...
		echo "<span class='text-muted h5'>" . ($trans->get_assign('timeoff', 'viewall', $user_id) ? "All Time" : date("F d, Y", strtotime($d1)) . "-" . date("F d, Y", strtotime($d2))) . "</span>";
	...
	case 'ot':
		...
		*/
		...
		if(!in_array($trans->getjobinfo($user_id, 'jrec_position'), ['EC-SAPPHIRE', 'EC-PEARL', 'EC-DIAMOND', 'EC-SAPPHIRE', 'EC', 'EC-T', 'EC-A', 'SIC', 'SIC-A']) || $trans->get_assign('timeoff', 'viewall', $user_id)){
			echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_id."\" data-reqchange=\"0\" data-target=\"#otmodal\"><i class='fa fa-plus'></i></button>";
		}
		// }
		...
		if(in_array($trans->getjobinfo($user_id, 'jrec_position'), ['EC-SAPPHIRE', 'EC-PEARL', 'EC-DIAMOND', 'EC-SAPPHIRE', 'EC', 'EC-T', 'EC-A', 'SIC', 'SIC-A']) || $trans->getjobinfo($user_id, 'jrec_department') == 'SLS' || $trans->get_assign('timeoff', 'viewall', $user_id)){
			...
			// echo "<a class='nav-link' id='ot_dtr-tab' data-toggle='tab' href='#ot_dtr' role='tab' aria-controls='ot_dtr' aria-selected='true' data-reqemp=\"".$user_id."\">DTR</a>";
			echo "</li>";
		}
	...
	case 'offset':
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_id."\" data-reqchange=\"0\" data-target=\"#offsetmodal\"><i class='fa fa-plus'></i></button>";
	...
	case 'training':
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_id."\" data-reqchange=\"0\" data-reqtype=\"".$load."\" data-target=\"#activitymodal\"><i class='fa fa-plus'></i></button>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				echo "<tr>";
				echo "<td class='text-center align-middle'>";
				if($v['emp_no'] != $user_id && in_array($v['emp_no'], $user_assign_arr3)){
		...
		if(!empty($arr['approved'])){
			foreach ($arr['approved'] as $k => $v) {
				...
				echo "<td>";
				if($trans->get_assign('b_forms', 'viewall', $user_id)){
	...
	case 'dhd':
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_id."\" data-reqchange=\"0\" data-target=\"#".$load."modal\"><i class='fa fa-plus'></i></button>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				...
				if($v['empno'] != $user_id && (in_array($v['empno'], $user_assign_arr2) || in_array($v['empno'], $user_assign_list_sic_dhd_arr))){
	...
	case 'restday-application':
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_id."\" data-target=\"#restdaymodal\"><i class='fa fa-plus'></i></button>";
		echo "<ul class='nav nav-tabs' id='" . $load . "stattab' role='tablist'>";
		echo "<li class='nav-item'>";
		echo "<a class='nav-link active' id='" . $load . "_pending-tab' data-toggle='tab' href='#" . $load . "_pending' role='tab' aria-controls='" . $load . "_pending' aria-selected='true'>Pending</a>";
		echo "</li>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				echo "<tr>";
				echo "<td class='text-center align-middle'>";
				if($v['rd_emp'] != $user_id && in_array($v['rd_emp'], $user_assign_arr_rd)){
				...
				if($v['rd_emp'] != $user_id && in_array($v['rd_emp'], $user_assign_arr_rd) && $approver == 1){
	...
	case 'dtr':
		...
		$query = $con1->prepare($sql);
		$query->execute([ $d1, $d2, $user_assign_list, ($trans->get_assign('manualdtr', 'viewall', $user_id) ? 1 : 0) ]);
		$arr = [];
		foreach ($query->fetchall(PDO::FETCH_ASSOC) as $k => $v) {
			$v['dtrtype'] = "sti";
			$arr[strtolower($v['dtr_stat'])][] = $v;
		...
		$query = $con1->prepare($sql);
		$query->execute([ $d1, $d2, $d1, $d2, $user_assign_list, ($trans->get_assign('manualdtr', 'viewall', $user_id) ? 1 : 0), $user_id ]);
		foreach ($query->fetchall(PDO::FETCH_ASSOC) as $k => $v) {
			$arr['req'][] = $v;
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_id."\" data-target=\"#dtrbatchmodal\"><i class='fa fa-plus'></i></button>";
		echo "<ul class='nav nav-tabs' id='" . $load . "stattab' role='tablist'>";
		echo "<li class='nav-item'>";
		...
		if(!empty($pending_dtr)){
			...
			foreach ($pending_dtr as $k => $v) {
				echo "<tr class=''>";
				echo "<td class='text-center align-middle'>";
				if($v['emp_no'] != $user_id && (in_array($v['emp_no'], $user_assign_arr) || $trans->get_assign('manualdtr', 'viewall', $user_id))){
				...
				if($v['emp_no'] != $user_id && ((in_array($v['emp_no'], $user_assign_arr) && $approver == 1) || in_array($v['emp_no'], $user_assign_list_sic_dhd_arr))){
				...
				}elseif($v['emp_no'] != $user_id && $trans->get_assign('manualdtr', 'check', $user_id)){
		...
		if(!empty($checked_dtr)){
			...
			foreach ($checked_dtr as $k => $v) {
				...
				echo "<td class='text-center align-middle'>";
				if($v['emp_no'] != $user_id && in_array($v['emp_no'], $user_assign_arr)){
					echo "<input type='checkbox' style='width: 20px; height: 20px;' class='approvechkitem' data-reqid=\"".$v['id']."\" data-reqemp=\"".$v['emp_no']."\" data-dtrtype=\"".$v['dtrtype']."\">";
					$tochk ++;
				}
				echo "</td>";
				...
				if($v['emp_no'] != $user_id && in_array($v['emp_no'], $user_assign_arr) && $approver == 1){
					echo "<button type=\"button\" class=\"reqapprove btn btn-outline-primary btn-sm m-1\" data-reqtype=\"".$load."\" data-reqid=\"".$v['id']."\" data-reqemp=\"".$v['emp_no']."\" data-dtrtype=\"".$v['dtrtype']."\"><i class='fa fa-check'></i></button>";
					echo "<button type=\"button\" class=\"reqdeny btn btn-outline-danger btn-sm m-1\" title='Deny' data-reqtype=\"".$load."\" data-reqid=\"".$v['id']."\" data-reqemp=\"".$v['emp_no']."\" data-dtrtype=\"".$v['dtrtype']."\"><i class='fa fa-times'></i></button>";
				}
				echo "</td>";
		...
		if(!empty($arr['req'])){
			foreach ($arr['req'] as $k => $v) {
				...
				echo "<td>";
				if($v['du_stat'] == 'pending'){
					if (in_array($v['du_empno'], $user_assign_arr) && $v['du_empno'] != $user_id && $approver == 1) {
	...
	case 'gatepass':
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_id."\" data-reqchange=\"0\" data-reqtype=\"Official\" data-target=\"#gatepassmodal\"><i class='fa fa-plus'></i></button>";
		echo "<ul class='nav nav-tabs' id='" . $load . "stattab' role='tablist'>";
		echo "<li class='nav-item'>";
		echo "<a class='nav-link active' id='" . $load . "_pending-tab' data-toggle='tab' href='#" . $load . "_pending' role='tab' aria-controls='" . $load . "_pending' aria-selected='true'>Pending</a>";
		echo "</li>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				echo "<tr>";
				echo "<td class='text-center align-middle'>";
				if($v['emp_no'] != $user_id && in_array($v['emp_no'], $user_assign_arr4)){
					echo "<input type='checkbox' style='width: 20px; height: 20px;' class='approvechkitem' data-reqid=\"".$v['id']."\" data-reqemp=\"".$v['emp_no']."\">";
					$tochk ++;
				}
				echo "</td>";
	...
	case 'notify':
		...
		// if($trans->get_assign('manualdtr','approve',$user_id)){
	...
	case 'break':
		...
		// $emplist = $trans->check_auth($user_id, 'break');
		$emplist = $trans->check_auth($user_id, 'DTR');
		...
		$query = $con1->prepare($sql);
		$query->execute([ $d1, $d2, $emplist, $user_id ]);
		$arr = [];
		foreach ($query->fetchall(PDO::FETCH_ASSOC) as $k => $v) {
			$arr[ $v['brv_stat'] ][] = $v;
		}

		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqdate=\"".date("Y-m-d")."\" data-reqemp=\"".$user_id."\" data-reqtype=\"".$load."\" data-target=\"#empbreakModal\"><i class='fa fa-plus'></i></button>";
		echo "<ul class='nav nav-tabs' id='" . $load . "stattab' role='tablist'>";
		echo "<li class='nav-item'>";
		echo "<a class='nav-link active' id='" . $load . "_pending-tab' data-toggle='tab' href='#" . $load . "_pending' role='tab' aria-controls='" . $load . "_pending' aria-selected='true'>Pending</a>";
		echo "</li>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				echo "<tr>";
				echo "<td class='text-center align-middle'>";
				if($v['brv_empno'] != $user_id && in_array($v['brv_empno'], $emplist)){
				...
				if($v['brv_empno'] != $user_id && in_array($v['brv_empno'], $emplist) && $approver == 1){
