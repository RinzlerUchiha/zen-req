...
switch ($load) {
	case 'month':
		...
		if($e == 'all' && $trans->get_assign('manpower', 'viewemp', $user_empno)){
			$e = $user_assign_list;
		}
			...
			for ($x = 0; $x < 7; $x++) { 
				...
				if($days[$x] == date("D", strtotime($curdt)) && $curmonth == date("F", strtotime($curdt)) && $curdt <= $enddt){
					...
					if($e == "all" || $e == "" || count(explode(",", $e)) > 1){
						...
						$otcnt = count(array_filter($dtr_ot, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr2, $user_empno) {
						...
						$leavecnt = count(array_filter($leavearr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr2, $user_empno) {
									    return isset($v[$curdt]) && $v[$curdt]['status'] == 'pending' && in_array($k, $user_assign_arr2) && ((isset($prev_outlet[$k]) && $o == $prev_outlet[$k]) || (isset($targets[$k][date("Y-m", strtotime($curdt))]) && $o == $targets[$k][date("Y-m", strtotime($curdt))]) || $o == '');
									}, ARRAY_FILTER_USE_BOTH));
						$travelcnt = count(array_filter($travelarr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr3, $user_empno) {
						...
						$trainingcnt = count(array_filter($trainingarr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr3, $user_empno) {
									    return isset($v[$curdt]) && $v[$curdt]['status'] == 'pending' && in_array($k, $user_assign_arr3) && ((isset($prev_outlet[$k]) && $o == $prev_outlet[$k]) || (isset($targets[$k][date("Y-m", strtotime($curdt))]) && $o == $targets[$k][date("Y-m", strtotime($curdt))]) || $o == '');
									}, ARRAY_FILTER_USE_BOTH));
						$oscnt = count(array_filter($osarr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr2, $user_empno) {
									    return isset($v[$curdt]) && $v[$curdt]['status'] == 'pending' && in_array($k, $user_assign_arr2) && ((isset($prev_outlet[$k]) && $o == $prev_outlet[$k]) || (isset($targets[$k][date("Y-m", strtotime($curdt))]) && $o == $targets[$k][date("Y-m", strtotime($curdt))]) || $o == '');
									}, ARRAY_FILTER_USE_BOTH));
						$drdcnt = count(array_filter($drdarr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr2, $user_empno) {
									    return isset($v[$curdt]) && $v[$curdt]['status'] == 'pending' && in_array($k, $user_assign_arr2) && ((isset($prev_outlet[$k]) && $o == $prev_outlet[$k]) || (isset($targets[$k][date("Y-m", strtotime($curdt))]) && $o == $targets[$k][date("Y-m", strtotime($curdt))]) || $o == '');
									}, ARRAY_FILTER_USE_BOTH));
						$dhdcnt = count(array_filter($dhdarr, function($v, $k) use($curdt, $o, $prev_outlet, $targets, $user_assign_arr2, $user_empno) {
									    return isset($v[$curdt]) && $v[$curdt]['status'] == 'pending' && in_array($k, $user_assign_arr2) && ((isset($prev_outlet[$k]) && $o == $prev_outlet[$k]) || (isset($targets[$k][date("Y-m", strtotime($curdt))]) && $o == $targets[$k][date("Y-m", strtotime($curdt))]) || $o == '');
									}, ARRAY_FILTER_USE_BOTH));
	...
	case 'dtrreport':
		...
		if($emp_arr == 'all' && $trans->get_assign('manpower', 'viewemp', $user_empno)){
	...
	case 'leave':
		...
		if($trans->get_assign('timeoff', 'viewall', $user_empno)){
			$sql = "SELECT
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_empno."\" data-reqchange=\"0\" data-reqtype=\"\" data-reqreason=\"\" data-reqmtype=\"\" data-reqdays=\"\" data-reqstart=\"\" data-reqreturn=\"\" data-target=\"#leavemodal\"><i class='fa fa-plus'></i></button>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				...
				if($v['la_empno'] != $user_empno && in_array($v['la_empno'], $user_assign_arr2)){
		...
		echo "<span class='text-muted h5'>" . ($trans->get_assign('timeoff', 'viewall', $user_empno) ? "All Time" : date("F d, Y", strtotime($d1)) . "-" . date("F d, Y", strtotime($d2))) . "</span>";
		...
		if(!empty($arr['approved'])){
			foreach ($arr['approved'] as $k => $v) {
				...
				if($v['la_empno'] == $user_empno && empty($v['rtw_id'])){
		...
		if(!empty($rtw_list)){
			foreach ($rtw_list as $k => $v) {
				...
				foreach ($v as $k2 => $v2) {
					...
					echo "<td>";
					if($v2['rtw_empno'] == $user_empno && $v2['rtw_stat'] == 'pending'){
					...
					if($v2['rtw_empno'] == $user_empno && in_array($v2['rtw_empno'], $user_assign_arr2) && $v2['rtw_stat'] == 'pending'){
	...
	case 'ot':
		...
		*/
		...
		// if(!($timekeeping->superflexi($user_empno, $trans->getjobinfo($user_empno, 'jrec_department'), $trans->getjobinfo($user_empno, 'jrec_company'), date("Y-m-d")) == true || in_array($trans->getjobinfo($user_empno, 'jrec_position'), ['EC-SAPPHIRE', 'EC-PEARL', 'EC-DIAMOND', 'EC-SAPPHIRE', 'EC', 'EC-T', 'EC-A', 'SIC', 'SIC-A']))){
		if(!in_array($trans->getjobinfo($user_empno, 'jrec_position'), ['EC-SAPPHIRE', 'EC-PEARL', 'EC-DIAMOND', 'EC-SAPPHIRE', 'EC', 'EC-T', 'EC-A', 'SIC', 'SIC-A']) || $trans->get_assign('timeoff', 'viewall', $user_empno)){
			echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_empno."\" data-reqchange=\"0\" data-target=\"#otmodal\"><i class='fa fa-plus'></i></button>";
		}
		// }

		echo "<ul class='nav nav-tabs' id='otstattab' role='tablist'>";
		if(in_array($trans->getjobinfo($user_empno, 'jrec_position'), ['EC-SAPPHIRE', 'EC-PEARL', 'EC-DIAMOND', 'EC-SAPPHIRE', 'EC', 'EC-T', 'EC-A', 'SIC', 'SIC-A']) || $trans->getjobinfo($user_empno, 'jrec_department') == 'SLS' || $trans->get_assign('timeoff', 'viewall', $user_empno)){
			echo "<li class='nav-item'>";
			...
			// echo "<a class='nav-link' id='ot_dtr-tab' data-toggle='tab' href='#ot_dtr' role='tab' aria-controls='ot_dtr' aria-selected='true' data-reqemp=\"".$user_empno."\">DTR</a>";
			echo "</li>";
		}
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				foreach ($v as $k2 => $v2) {
					echo "<tr>";
					echo "<td class='text-center align-middle'>";
					if($v2['empno'] != $user_empno && in_array($v2['empno'], $user_assign_arr2)){
		...
		if(!empty($arr['approved'])){
			foreach ($arr['approved'] as $k => $v) {
				foreach ($v as $k2 => $v2) {
					...
					if($user_empno == $v2['empno'] && !($timekeeping->superflexi($user_empno, $trans->getjobinfo($user_empno, 'jrec_department'), $trans->getjobinfo($user_empno, 'jrec_company'), date("Y-m-d")) == true || in_array($trans->getjobinfo($user_empno, 'jrec_position'), ['EC-SAPPHIRE', 'EC-PEARL', 'EC-DIAMOND', 'EC-SAPPHIRE', 'EC', 'EC-T', 'EC-A', 'SIC', 'SIC-A']))){
						echo "<button type=\"button\" class=\"btn btn-outline-secondary btn-sm m-1\" title='Edit' 
						data-toggle=\"modal\"
						data-target=\"#editotmodal\"
						data-reqact=\"add\" 
	...
	case 'offset':
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_empno."\" data-reqchange=\"0\" data-target=\"#offsetmodal\"><i class='fa fa-plus'></i></button>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				echo "<tr>";
				echo "<td class='text-center align-middle'>";
				if($v['empno'] != $user_empno && in_array($v['empno'], $user_assign_arr2)){
	...
	case 'training':
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_empno."\" data-reqchange=\"0\" data-reqtype=\"".$load."\" data-target=\"#activitymodal\"><i class='fa fa-plus'></i></button>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				echo "<tr>";
				echo "<td class='text-center align-middle'>";
				if($v['emp_no'] != $user_empno && in_array($v['emp_no'], $user_assign_arr3)){
		...
		if(!empty($arr['approved'])){
			foreach ($arr['approved'] as $k => $v) {
				...
				echo "<td>";
				if($trans->get_assign('b_forms', 'viewall', $user_empno)){
	...
	case 'dhd':
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_empno."\" data-reqchange=\"0\" data-target=\"#".$load."modal\"><i class='fa fa-plus'></i></button>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				...
				if($v['empno'] != $user_empno && (in_array($v['empno'], $user_assign_arr2) || in_array($v['empno'], $user_assign_list_sic_dhd_arr))){
	...
	case 'restday-application':
		...
		$approver = $trans->get_assign('manualdtr','approve',$user_empno) ? 1 : 0;
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_empno."\" data-target=\"#restdaymodal\"><i class='fa fa-plus'></i></button>";
		echo "<ul class='nav nav-tabs' id='" . $load . "stattab' role='tablist'>";
		echo "<li class='nav-item'>";
		echo "<a class='nav-link active' id='" . $load . "_pending-tab' data-toggle='tab' href='#" . $load . "_pending' role='tab' aria-controls='" . $load . "_pending' aria-selected='true'>Pending</a>";
		echo "</li>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				echo "<tr>";
				echo "<td class='text-center align-middle'>";
				if($v['rd_emp'] != $user_empno && in_array($v['rd_emp'], $user_assign_arr_rd)){
				...
				if($v['rd_emp'] != $user_empno && in_array($v['rd_emp'], $user_assign_arr_rd) && $approver == 1){
	...
	case 'dtr':
		...
		$query = $con1->prepare($sql);
		$query->execute([ $d1, $d2, $user_assign_list, ($trans->get_assign('manualdtr', 'viewall', $user_empno) ? 1 : 0) ]);
		$arr = [];
		foreach ($query->fetchall(PDO::FETCH_ASSOC) as $k => $v) {
			$v['dtrtype'] = "sti";
			$arr[strtolower($v['dtr_stat'])][] = $v;
		...
		$query = $con1->prepare($sql);
		$query->execute([ $d1, $d2, $d1, $d2, $user_assign_list, ($trans->get_assign('manualdtr', 'viewall', $user_empno) ? 1 : 0), $user_empno ]);
		foreach ($query->fetchall(PDO::FETCH_ASSOC) as $k => $v) {
			$arr['req'][] = $v;
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_empno."\" data-target=\"#dtrbatchmodal\"><i class='fa fa-plus'></i></button>";
		echo "<ul class='nav nav-tabs' id='" . $load . "stattab' role='tablist'>";
		echo "<li class='nav-item'>";
		...
		if(!empty($pending_dtr)){
			...
			foreach ($pending_dtr as $k => $v) {
				echo "<tr class=''>";
				echo "<td class='text-center align-middle'>";
				if($v['emp_no'] != $user_empno && (in_array($v['emp_no'], $user_assign_arr) || $trans->get_assign('manualdtr', 'viewall', $user_empno))){
				...
				}elseif($v['emp_no'] != $user_empno && $trans->get_assign('manualdtr', 'check', $user_empno)){
		...
		if(!empty($checked_dtr)){
			...
			foreach ($checked_dtr as $k => $v) {
				...
				echo "<td class='text-center align-middle'>";
				if($v['emp_no'] != $user_empno && in_array($v['emp_no'], $user_assign_arr)){
					echo "<input type='checkbox' style='width: 20px; height: 20px;' class='approvechkitem' data-reqid=\"".$v['id']."\" data-reqemp=\"".$v['emp_no']."\" data-dtrtype=\"".$v['dtrtype']."\">";
					$tochk ++;
				}
				echo "</td>";
				...
				if($v['emp_no'] != $user_empno && in_array($v['emp_no'], $user_assign_arr) && $approver == 1){
					echo "<button type=\"button\" class=\"reqapprove btn btn-outline-primary btn-sm m-1\" data-reqtype=\"".$load."\" data-reqid=\"".$v['id']."\" data-reqemp=\"".$v['emp_no']."\" data-dtrtype=\"".$v['dtrtype']."\"><i class='fa fa-check'></i></button>";
					echo "<button type=\"button\" class=\"reqdeny btn btn-outline-danger btn-sm m-1\" title='Deny' data-reqtype=\"".$load."\" data-reqid=\"".$v['id']."\" data-reqemp=\"".$v['emp_no']."\" data-dtrtype=\"".$v['dtrtype']."\"><i class='fa fa-times'></i></button>";
				}
				echo "</td>";
	...
	case 'gatepass':
		...
		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqemp=\"".$user_empno."\" data-reqchange=\"0\" data-reqtype=\"Official\" data-target=\"#gatepassmodal\"><i class='fa fa-plus'></i></button>";
		echo "<ul class='nav nav-tabs' id='" . $load . "stattab' role='tablist'>";
		echo "<li class='nav-item'>";
		echo "<a class='nav-link active' id='" . $load . "_pending-tab' data-toggle='tab' href='#" . $load . "_pending' role='tab' aria-controls='" . $load . "_pending' aria-selected='true'>Pending</a>";
		echo "</li>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				echo "<tr>";
				echo "<td class='text-center align-middle'>";
				if($v['emp_no'] != $user_empno && in_array($v['emp_no'], $user_assign_arr4)){
					echo "<input type='checkbox' style='width: 20px; height: 20px;' class='approvechkitem' data-reqid=\"".$v['id']."\" data-reqemp=\"".$v['emp_no']."\">";
					$tochk ++;
				}
				echo "</td>";
				...
				// if($user_empno == '045-2017-068'){
				// 	echo "<td><img src='/hris2/img/gp_attachment/".$v['gp_attachment']."' width='100%' style='max-height: 300px;'>={$_SERVER['DOCUMENT_ROOT']}/hris2/img/gp_attachment/".$v['gp_attachment']."=</td>";
				// }else{
					echo "<td>" . (!empty($v['gp_attachment']) && file_exists("{$_SERVER['DOCUMENT_ROOT']}/hris2/img/gp_attachment/".$v['gp_attachment']) ? "<!-- <a href='/hris2/img/gp_attachment/".$v['gp_attachment']."' target='_blank'>View Attachment</a> --><embed src='/hris2/img/gp_attachment/".$v['gp_attachment']."' width='' style='min-height: 200px; max-height: 300px;'>" : "") . "</td>";
				// }
		...
		if(!empty($arr['approved'])){
			foreach ($arr['approved'] as $k => $v) {
				...
				echo "<td>" . $v['type'] . "</td>";
				echo "<td>" . nl2br($v['purpose']) . "</td>";
				echo "<td>" . (file_exists("/hris2/img/gp_attachment/".$v['gp_attachment']) ? "<!-- <a href='/hris2/img/dtr_attachment/".$v['gp_attachment']."' target='_blank'>View Attachment</a> --><img src='/hris2/img/dtr_attachment/".$v['gp_attachment']."' width='100%' style='max-height: 200px;'>" : "") . "</td>";
				echo "<td>" . date("Y-m-d", strtotime($v['date_created'])) . "</td>";
				// echo "<td>" . get_emp_name($v['rd_approvedby']) . "</td>";
				// echo "<td>" . date("Y-m-d", strtotime($v['rd_approveddt'])) . "</td>";
				echo "<td>";
	...
	case 'notify':
		...
		// if($trans->get_assign('manualdtr','approve',$user_empno)){
	...
	case 'restday':
		...
		$emplist = $trans->check_auth($user_empno, 'RD');
	...
	case 'break':
		...
		// $emplist = $trans->check_auth($user_empno, 'break');
		$emplist = $trans->check_auth($user_empno, 'DTR');
		...
		$query = $con1->prepare($sql);
		$query->execute([ $d1, $d2, $emplist, $user_empno ]);
		$arr = [];
		foreach ($query->fetchall(PDO::FETCH_ASSOC) as $k => $v) {
			$arr[ $v['brv_stat'] ][] = $v;
		}

		echo "<button type=\"button\" class=\"btn btn-outline-primary btn-sm m-1 btnadd\" title='Add' data-toggle=\"modal\" data-reqact=\"add\" data-reqid=\"\" data-reqdate=\"".date("Y-m-d")."\" data-reqemp=\"".$user_empno."\" data-reqtype=\"".$load."\" data-target=\"#empbreakModal\"><i class='fa fa-plus'></i></button>";
		echo "<ul class='nav nav-tabs' id='" . $load . "stattab' role='tablist'>";
		echo "<li class='nav-item'>";
		echo "<a class='nav-link active' id='" . $load . "_pending-tab' data-toggle='tab' href='#" . $load . "_pending' role='tab' aria-controls='" . $load . "_pending' aria-selected='true'>Pending</a>";
		echo "</li>";
		...
		if(!empty($arr['pending'])){
			foreach ($arr['pending'] as $k => $v) {
				echo "<tr>";
				echo "<td class='text-center align-middle'>";
				if($v['brv_empno'] != $user_empno && in_array($v['brv_empno'], $emplist)){
				...
				if($v['brv_empno'] != $user_empno && in_array($v['brv_empno'], $emplist) && $approver == 1){
