...
switch ($action) {
	...
	case 'APPROVED':
		...
		// $sign=$_POST['sign'] ?? '';
		$dtworked="";
		$sql=$con1->prepare("UPDATE tbl_edtr_hours SET dtr_stat=?, approvedby=?, dt_approved=? WHERE emp_no=? AND id=?");
		if($sql->execute(array("APPROVED",$user_id,date("Y-m-d H:i:s"),$empno,$id))){
			echo "1";
	...
	case 'BATCH_APPROVED':
		...
		foreach ($requests as $req) {
		  ...
		  $sql = $con1->prepare("UPDATE tbl_edtr_hours SET dtr_stat=?, approvedby=?, dt_approved=? WHERE emp_no=? AND id=?");
		  $sql->execute(["APPROVED", $user_id, date("Y-m-d H:i:s"), $empno, $id]);

		  // Log the approval
		  $action1 = 'Batch approved activity ID: ' . $id . ', Emp No: ' . $empno;
		  $sql1a = $con1->prepare("INSERT INTO tbl_edtr_logs (emp_no, activity, date_time) VALUES (?, ?, ?)");
		  $sql1a->execute([$user_id, $action1, date("Y-m-d H:i:s")]);

		  // Optional: Clear total hours cache or update
		  $sql1 = $con1->prepare("SELECT emp_no, date_dtr FROM tbl_edtr_hours WHERE id = ? AND emp_no = ?");
