		...
		case 'add':
				...
				try {
			        ...
			        if($id==""){
				        ...
				        if($sql->execute(array($ecf_no, $emp, get_emp_name_init($emp), $company, $dept, $outlet, $pos, $empstat, $lastday, $separation, $user_empno, date("Y-m-d H:i:s"), $hold_date, "draft"))){
				        	$id=$hr_pdo->lastInsertId();
				        	echo "1";

				        	_log("Created a clearance request for ".get_emp_name_init($emp).". ID: ".$id." Data:[".implode(", ", [$emp, get_emp_name_init($emp), $company, $dept, $outlet, $pos, $empstat, $lastday, $separation, $user_empno, date("Y-m-d H:i:s"), $hold_date, "draft"]));
				        }
				        ...
				        if($sql->execute(array($ecf_no, $emp, get_emp_name_init($emp), $company, $dept, $outlet, $pos, $empstat, $lastday, $separation, $user_empno, date("Y-m-d"), $hold_date, "draft", $id))){
				        	echo "1";

				        	_log("Updated a clearance request. ID: ".$id." New Data:[".implode(", ", [$ecf_no, $emp, get_emp_name_init($emp), $company, $dept, $outlet, $pos, $empstat, $lastday, $separation, $user_empno, date("Y-m-d"), $hold_date, "draft"]));
				        }
					}
		...
		case 'post':
				...
				try {
					...
					if($id==""){
				        ...
				        $sql=$hr_pdo->prepare("INSERT INTO db_ecf2.tbl_request (ecf_no, ecf_empno, ecf_name, ecf_company, ecf_dept, ecf_outlet, ecf_pos, ecf_empstatus, ecf_lastday, ecf_separation, ecf_reqby, ecf_reqdate, ecf_salholddt, ecf_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
				        if($sql->execute(array($ecf_no, $emp, get_emp_name_init($emp), $company, $dept, $outlet, $pos, $empstat, $lastday, $separation, $user_empno, date("Y-m-d"), $hold_date, "pending"))){
				        	$id=$hr_pdo->lastInsertId();
				        	echo "1";
				        	_log("Created a clearance request for ".get_emp_name_init($emp).". ID: ".$id." Data:[".implode(", ", [$emp, get_emp_name_init($emp), $company, $dept, $outlet, $pos, $empstat, $lastday, $separation, $user_empno, date("Y-m-d H:i:s"), $hold_date, "pending"]));
				        }
					}else{
						$sql=$hr_pdo->prepare("UPDATE db_ecf2.tbl_request  SET ecf_no=?, ecf_empno=?, ecf_name=?, ecf_company=?, ecf_dept=?, ecf_outlet=?, ecf_pos=?, ecf_empstatus=?, ecf_lastday=?, ecf_separation=?, ecf_reqby=?, ecf_reqdate=?, ecf_salholddt=?, ecf_status=? WHERE ecf_id=?");
				        if($sql->execute(array($ecf_no, $emp, get_emp_name_init($emp), $company, $dept, $outlet, $pos, $empstat, $lastday, $separation, $user_empno, date("Y-m-d"), $hold_date, "pending", $id))){
				        	echo "1";
				        	_log("Updated a clearance request. ID: ".$id." New Data:[".implode(", ", [$ecf_no, $emp, get_emp_name_init($emp), $company, $dept, $outlet, $pos, $empstat, $lastday, $separation, $user_empno, date("Y-m-d"), $hold_date, "pending"]));
				        }
					}
				...
				try {
					...
					foreach ($catset as $key => $value) {
						...
						$sql2=$hr_pdo->prepare("UPDATE db_ecf2.tbl_req_category SET catstat_dtchecked=?, catstat_sign=?, catstat_stat=? WHERE catstat_id=?");
				       	...
				       	_log("Signed a clearance request. ID: ".$value[0]);
					}
