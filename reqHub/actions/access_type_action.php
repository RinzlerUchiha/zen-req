 ...
 * File: /zen/reqHub/actions/access_type_action.php
 */
...
try {
    switch ($action) {
        ...
        case 'deleteRole':
            ...
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM role_permissions WHERE role_id IN (SELECT id FROM roles WHERE name = ?)");
            $stmt->execute([$role]);
            if ($stmt->fetchColumn() > 0)
        ...
        case 'deleteModule':
            ...
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM module_actions WHERE module_id IN (SELECT id FROM modules WHERE name = ?)");
            $stmt->execute([$module]);
            if ($stmt->fetchColumn() > 0)
        ...
        case 'deleteAction':
            ...
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM module_actions WHERE action_id IN (SELECT id FROM actions WHERE name = ?)");
            $stmt->execute([$name]);
            if ($stmt->fetchColumn() > 0)
