...
class PCFR
{
    ...
    public static function GetReplenishList($user_id) {
        ...
        if ($conn) {
            $stmt = $conn->prepare("SELECT * FROM tbl_replenish 
                ...
                AND repl_status IN ('h-approved','checked','f-returned','f-approved','deposited','received')
               ORDER BY 
               (CASE
    ...
    public static function GetReplRequest($outlet) {
        ...
        if ($conn) {
            $stmt = $conn->prepare("SELECT repl_expense as expense,repl_outlet,repl_no  FROM tbl_replenish
                WHERE repl_outlet = ?
