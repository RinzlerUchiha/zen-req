...
$arr_job = [];
$sql1 = $hr_db->query("SELECT jd_code, jd_title, jd_stat FROM tbl_jobdescription WHERE jd_company IN ('SJI');");
foreach ($sql1 as $r1) {
	...
	<div class="panel panel-default">
		<div class="panel-heading">
			...
			<span class="pull-right">
				<a href="?page=sji_pa_settings&type=qualitative" class="btn btn-default btn-mini" style="font-weight: bold;">QUALITATIVE SETTING</a>
			</span>
		</div>
		<div class="panel-body" style="margin: 10px;">
			<div class="" style="width: 30%;display: flex;color: #000;">
				<select class="form-control selectpicker" data-live-search="true" title="Select" id="selectqty">
					<?php
						$initial_pos = '';
						// $sql1 = $hr_db->query("SELECT DISTINCT qtyset_for FROM tbl_paqty_setting");
				...
				<button class="btn btn-default btn-mini" onclick="loadqtyset('qty');"><i class="fa fa-search"></i></button>
			</div>
			...
			<div id="div_qty" style="max-width: 100%; overflow-y: auto;">
				<table class="table table-bordered" width="100%" qtypos="<?=$initial_pos?>">
					...
					<tbody>
						<?php
							$sql1 = $hr_db->query("SELECT * FROM tbl_paqty_setting WHERE qtyset_for = '$initial_pos';");
							// $sql1->execute();
							...
							foreach ($sql1 as $r1) {
								...
								echo "<td>
										<button class=\"btn btn-default btn-mini btneditqty\"><i class=\"fa fa-edit\"></i></button>
										<button style='display: none;' class=\"btn btn-primary btn-mini btnsaveqty\" onclick=\"saveqty($(this).parents('tr'))\"><i class=\"fa fa-check\"></i></button>
										<button class=\"btn btn-danger btn-mini btndelqty\" onclick=\"del_qty($(this).parents('tr'));\"><i class=\"fa fa-times\"></i></button>
									</td>";
								echo "</tr>";
							}
						?>
				...
				<button class="btn btn-primary btn-mini" onclick="addrowqty()">Add</button>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	function loadqtyset(type1) {
		$.post("/Portal/pasji/pages/sji_pa_setting_data.php",{ getdata: $("#selectqty").val(), type: type1 }, function(data1){
	...
	function addrowqty() {
		...
		txt1 += "<td><button style='display: none;' class=\"btn btn-default btn-mini btneditqty\"><i class=\"fa fa-edit\"></i></button><button class=\"btn btn-primary btn-mini btnsaveqty\" onclick=\"saveqty($(this).parents('tr'))\"><i class=\"fa fa-check\"></i></button><button class=\"btn btn-danger btn-mini\" onclick=\"del_qty($(this).parents('tr'))\"><i class=\"fa fa-times\"></i></button></td>";
		txt1 += "</tr>";
...
<div class="container-fluid">
	<div class="panel panel-default">
		<div class="panel-heading">
			...
			<span class="pull-right">
				<a href="?page=sji_pa_settings&type=quantitative" class="btn btn-default btn-mini" style="font-weight: bold;">QUANTITATIVE SETTING</a>
			</span>
		</div>
		<div class="panel-body">
			<div class="">
				<select class="form-control selectpicker" data-live-search="true" title="Select" id="selectqlty">
					<?php
						$initial_pos = '';
						$sql1 = $hr_db->query("SELECT DISTINCT qltyset_for FROM tbl_paqlty_setting");
						foreach ($arr_job as $rk1 => $r1) {
							if($r1[1] == 'active'){
								$initial_pos = !$initial_pos ? $rk1 : $initial_pos;
								echo "<option value=\"" . $rk1 . "\" " . ($initial_pos == $rk1 ? "selected" : "") . ">" . $r1[0] . "</option>";
			...
			<div id="div_qlty" style="max-width: 100%; overflow-y: auto;">
				<table class="table table-bordered" width="100%" qltypos="<?=$initial_pos?>">
					...
					<tbody>
						<?php
							$sql1 = $hr_db->query("SELECT * FROM tbl_paqlty_setting WHERE qltyset_for = '$initial_pos';");
							...
							foreach ($sql1 as $r1) {
								...
								echo	"</div>
										<button class=\"btn btn-default btn-mini btnaddindicator pull-right\" style='display: none;'><i class=\"fa fa-plus\"></i></button>
									</td>";

								echo "<td>
										<button class=\"btn btn-default btn-mini btneditqlty\"><i class=\"fa fa-edit\"></i></button>
										<button style='display: none;' class=\"btn btn-primary btn-mini btnsaveqlty\" onclick=\"saveqlty($(this).parents('tr'))\"><i class=\"fa fa-check\"></i></button>
										<button class=\"btn btn-danger btn-mini\" onclick=\"del_qlty($(this).parents('tr'))\"><i class=\"fa fa-times\"></i></button></td>";
								echo "</tr>";
							}
						?>
					</tbody>
				</table>
				<button class="btn btn-primary btn-mini" onclick="addrowqlty()">Add</button>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	function loadqltyset(type1) {
		$.post("/Portal/pasji/pages/sji_pa_setting_data.php",{ getdata: $("#selectqlty").val(), type: type1 }, function(data1){
			$("#div_" + type1).html(data1);
		});
	}
	...
	function addrowqlty() {
		...
		txt1 += "<td><div class='divqltyindicator'><span><input type='text' class='qltyindicator' style=\"width: 100%;\" value=''></span></div><button class=\"btn btn-default btn-mini btnaddindicator pull-right\"><i class=\"fa fa-plus\"></i></button></td>";
		txt1 += "<td><button style='display: none;' class=\"btn btn-default btn-mini btneditqlty\"><i class=\"fa fa-edit\"></i></button><button class=\"btn btn-primary btn-mini btnsaveqlty\" onclick=\"saveqlty($(this).parents('tr'))\"><i class=\"fa fa-check\"></i></button><button class=\"btn btn-danger btn-mini\" onclick=\"del_qlty($(this).parents('tr'))\"><i class=\"fa fa-times\"></i></button></td>";
		txt1 += "</tr>";
