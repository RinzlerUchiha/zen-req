...
switch ($type) {
	case 'qty':
		...
		echo "</thead>";
		echo "<tbody>";
		$sql1 = $hr_pdo->query("SELECT * FROM tbl_paqty_setting WHERE qtyset_for = '$getdata';");
		foreach ($sql1 as $r1) {
		...
		echo "</tbody>";
		echo "</table>";
		echo "<button class=\"btn btn-primary\" onclick=\"addrowqty()\">Add</button>";
	...
	case 'qlty':
		...
		echo "</thead>";
		echo "<tbody>";
		$sql1 = $hr_pdo->query("SELECT * FROM tbl_paqlty_setting WHERE qltyset_for = '$getdata';");
		foreach ($sql1 as $r1) {
		...
		echo "</tbody>";
		echo "</table>";
		echo "<button class=\"btn btn-primary\" onclick=\"addrowqlty()\">Add</button>";
