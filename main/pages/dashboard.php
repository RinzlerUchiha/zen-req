...
if(date('d') >= 18 /*&& empty($_SESSION['eei_cnt'])*/){
  ...
  $eei_sql = $hr_pdo->query("SELECT COUNT(resp_id) AS cnt FROM {$DB_EEI}.tbl_response WHERE resp_empno = '" . $user_id . "' AND DATE_FORMAT(resp_date,'%Y-%m') = '" . date('Y-m') . "'");
          ...
          <?php foreach ($replenishEntries as $row){
                if ($row['repl_status'] == 'submit') {
                   echo '<li class="list-group-item" id="list-notif">
                            <a style="text-decoration:none;color: black;" href="https://teamtngc.com/zen/pcf/view_rrr?rliD='.$row["repl_no"].'" target="_blank">New PCF Request No. '.$row['repl_no'].' has been submitted. Please review and approve.</a>
                        </li>';
                }
            }
          ?>
