...
try {
    ...
    $check_stmt = $port_db->prepare("SELECT COUNT(*) FROM tbl201_inter_cert WHERE ic_empno = :employee AND ic_title = :certTitle AND ic_date = :certdate");
    ...
    if ($check_stmt->fetchColumn() > 0) {
        $stmt = $port_db->prepare("UPDATE tbl201_inter_cert SET 
            ic_empno = :employee,
            ic_title = :certTitle,
            ic_address = :certlocation,
            ic_date = :certdate,
            ic_speaker = :certspeak,
            ic_file = :certimage,
            ic_status = :currstatus
            WHERE cert_empno = :employee AND cert_title = :certTitle AND cert_date = :certdate");
    } else {
        $stmt = $port_db->prepare("INSERT INTO tbl201_inter_cert (ic_empno, ic_title, ic_address, ic_date, ic_speaker, ic_file, ic_status) 
