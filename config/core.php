...
{
    include(__DIR__ . "/mysqlhelper.php");
...
function fn_get_user_details($field)
{
    ...
    if (isset($_SESSION[SESSION_KEY])) {
        $hruid = $_SESSION[SESSION_KEY];
    }

    include(__DIR__ . "/mysqlhelper.php");

    try {
        $query = $mysqlhelper->query("SELECT $field FROM tbl_user2 WHERE U_ID = " . $mysqlhelper->quote($hruid));

        if ($query) {
            $rquery = $query->fetch(PDO::FETCH_NUM);
            if ($rquery && isset($rquery[0])) {
                return $rquery[0];
            }
        }
    } catch (PDOException $e) {
        error_log("DB Error in fn_get_user_details: " . $e->getMessage());
    }
...
function fn_get_user_info($field1)
{
    ...
    try {
        ...
        $stmt = $mysqlhelper->prepare($sql);
        $stmt->execute([':hruid' => $hruid]);

        $rquery = $stmt->fetch(PDO::FETCH_NUM);
        return $rquery[0] ?? null; // return null if no result
    } catch (PDOException $e) {
        error_log("fn_get_user_info error: " . $e->getMessage());
        return null;
    }
}
...
function fn_get_user_dept($field1)
{
    ...
    try {
        ...
        $rquery = $stmt->fetch(PDO::FETCH_NUM);
        return $rquery[0] ?? null;
    } catch (PDOException $e) {
        error_log("fn_get_user_dept error: " . $e->getMessage());
        return null;
    }
}
...
function fn_get_user_jobinfo($field)
{
    ...
    try {
        ...
        error_log("fn_get_user_jobinfo error: " . $e->getMessage());
        return null;
    }
}
...
function _jobinfo($empno, $f)
{
    ...
    $stmt = $mysqlhelper->prepare("SELECT $f FROM tbl201_jobinfo WHERE ji_empno = :empno");
...
function get_assign($mod, $indv, $empno, $sys = 'HRIS')
{
    ...
    $rquery = $query->fetch(PDO::FETCH_NUM);
    // $_SESSION['access-'.implode('-', [$mod,$indv,$empno,$sys])] = $rquery[0];
    return $rquery[0];
}

function get_assign_fbr($mod, $indv, $empno, $sys = 'FBR')
{
    include(__DIR__ . "/fbr-helper.php");
    if ($mod != '') {
        $system = $sys;
        $query = $mysqlhelper->query("SELECT COUNT(*)
                                        FROM tbl_sysassign a 
...
function check_auth($empno, $for, $dept = false)
{
    ...
    try {
        if ($dept === false) {
            $sql = "SELECT auth_assignation 
                    FROM tbl_dept_authority 
                    WHERE auth_emp = :empno AND auth_for = :authfor";
        } else {
            $sql = "SELECT auth_dept 
                    FROM tbl_dept_authority 
                    WHERE auth_emp = :empno AND auth_for = :authfor";
        }
        ...
        if ($rquery && isset($rquery[0]) && $rquery[0] !== '') {
            return str_replace("|", ",", $rquery[0]);
        }
    } catch (PDOException $e) {
        error_log("check_auth error: " . $e->getMessage());
    }

    return ''; // return empty string if no match or error
}
...
function ecfMsg($id)
{
    ...
    foreach (
        $mysqlhelper->query("SELECT DISTINCT
